(() => {
  ...
  function explicitDealText(item) {
    ...
    const strongCommerce = [
      ...
      /\bdiscounts?\b/i,
      /\bcoupons?\b/i,
      /\bpromo(?:s|tion|tions)?\b/i,
      /\bsale\b|\bsales\b|\bon sale\b/i,
      /\bprice drop\b/i,
      /\bdeal alert\b/i,
      /\bbest deal\b|\bbest deals\b/i,
      /\bsave\s+(?:\$|up to|\d)/i,
      /\b\d+\s*%\s*off\b/i,
      /\bunder\s+\$\d+/i,
      /\bprime day\b|\bblack friday\b|\bcyber monday\b/i,
      /\bofertas?\b|\bdescontos?\b|\bcupom\b|\bcupons\b|\bpromo[cç][aã]o\b/i,
      /\bmelhores notebooks\b|\bnotebooks? em oferta\b|\bnotebooks? baratos\b/i,
      /\bbuying guide\b|\broundup\b|\bbest laptops?\b|\bbest monitors?\b|\bbest gaming laptops?\b/i,
    ];

    return strongCommerce.some(pattern => pattern.test(title));
  }

  function isDeal(item) {
    // Deals is a commerce bucket, not a generic "low value" bucket.
    // Do not inspect summaries: summaries often mention pricing/value in
    // legitimate reviews and strategic market stories.
    if (item.is_review && !explicitDealText(item)) return false;
