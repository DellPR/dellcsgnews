(() => {
  ...
  function summarizeMetricRows(rows) {
    ...
    rows.forEach(item => {
      ...
      metric.high_signal += Number(item.score || 0) >= 80 ? 1 : 0;
      if (item.source) metric.top_sources[item.source] = (metric.top_sources[item.source] || 0) + 1;
  ...
  function renderPieChart(title, brands, valueKey, emptyLabel) {
    ...
    const paths = slices.map(row => {
      ...
      angle = end;
      return `<path d="${path}" fill="${row.color}"><title>${escapeHtml(row.brand)} ${Math.round((row.value / total) * 100)}%</title></path>`;
    }).join("");
    ...
    return `<section class="metric-panel pie-panel">
      ...
      <div class="pie-wrap">
        <svg class="pie-chart" viewBox="0 0 100 100" role="img" aria-label="${escapeHtml(title)}">${paths}</svg>
  ...
  function renderBrandMetrics() {
    ...
    const topSourceRows = Object.entries(topSources)
      ...
      .join("");
    const recentRows = rows.slice(0, 12).map(item => `<tr>
