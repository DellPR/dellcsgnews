(() => {
  ...
  const TOP_STORY_URL = "https://www.youtube.com/watch?v=leWiFw52PJ4";
  ...
  if (brandMetricsEl) {
    brandMetricsEl.addEventListener("click", event => {
      const btn = event.target.closest("[data-brand-jump]");
      if (!btn) return;
      jumpToBrandFeed(btn.dataset.brandJump, btn.dataset.metricKey);
    });
  }
