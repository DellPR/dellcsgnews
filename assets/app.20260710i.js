(() => {
  ...
  function explicitDealText(item) {
    ...
    // Buying guides and "best of" lists are commerce/listicle only when they are
    // not an actual product review/comparison video.
  ...
  function isDeal(item) {
    ...
    if (looksEditorialReview(item) && !/\/(?:deals?|offers?|coupons?)(?:\/|$|[?#-])/i.test(String(item.url || ""))) {
      return false;
    }
